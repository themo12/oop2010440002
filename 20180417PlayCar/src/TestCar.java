...
//		Car yourCar = new Car(); // class 안에 우리가 생성자를 만들어 놓지 않으면 자바에서 자동으로 생성자를 만들어주기 때문에
//		                         // 이전에는 오류가 안떴지만 우리가 class 안에 매개변수를 받는 생성자만 만들어 놓으면
//		                         // 자바가 더이상 매개변수가 없는 생성자(default constructor)를 자동적으로 만들어주지 않기 때문에 오류가 뜬다.
